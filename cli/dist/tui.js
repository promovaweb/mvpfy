...
import { scanProgress, tenancyLabel } from "./progress.js";
...
export async function openTui(project = process.cwd()) {
    ...
    await new Promise((resolve) => screen.once("destroy", resolve));
}
export class MvpfyTui {
    ...
    async start(screen) {
        ...
        await this.refresh();
        screen.once("destroy", () => { if (this.#poller)
    ...
    showTab(tab, render = true) {
        this.#tab = tab;
        if (!this.#body || !this.#screen)
            return;
        for (const child of [...this.#body.children])
            child.destroy();
        if (tab === "progress")
            this.renderProgress(this.#body);
        else if (tab === "areas")
            this.renderAreas(this.#body);
        else if (tab === "document")
            void this.renderDocument(this.#body);
        else if (tab === "skills")
            this.renderSkills(this.#body);
        else
            this.renderAbout(this.#body);
        if (render)
            this.#screen.render();
    }
    renderProgress(body) {
    ...
    async renderDocument(body) {
        try {
            ...
            blessed.box({ parent: body, content: "MVP.md ainda não existe. Use mvpfy install ou prepare o projeto com a skill mvpfy.", padding: { left: 2, top: 1 }, style: { fg: THEME.warning, bg: THEME.surface } });
        }
    }
    renderSkills(body) {
    ...
    async runSkillAction(action) {
        this.setStatus(`${action === "install" ? "Instalando" : "Atualizando"} skills…`);
        try {
            const output = await runSkills(action === "install" ? installArguments() : updateArguments(), this.project);
    ...
    setStatus(message) { this.#status?.setContent(message); this.#screen?.render(); }
}
function areaLine(area) { return `${symbol(area.status)} ${area.label.padEnd(18, " ")} ${String(area.percent).padStart(3, " ")}%`; }
function symbol(status) { return status === "complete" ? "✓" : status === "in_progress" ? "◐" : "○"; }
function bar(percent, width) { const filled = Math.round((percent * width) / 100); return `[${"█".repeat(filled)}${"░".repeat(Math.max(0, width - filled))}]`; }
