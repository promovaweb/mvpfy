 ...
 */
function projectFromParent(command: Command, fallback: string): string {
  if (fallback !== process.cwd()) return fallback;
  let ancestor = command.parent;
  while (ancestor) {
    const project = ancestor.opts<{ project?: unknown }>().project;
