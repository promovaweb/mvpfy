...
| `cli/src/version.ts` | Versão exibida pela CLI, mantida igual a `VERSION`. |
...
5. Atualize `cli/src/version.ts` com o mesmo valor.
6. Crie a seção `## [X.Y.Z] - AAAA-MM-DD` no topo de `CHANGELOG.md`.
7. Explique a alteração e o efeito para a pessoa que usa o MVPFy.
8. Rode as verificações locais.
9. Faça commit e push na `main`.
...
7. publica o pacote raiz `@promovaweb/mvpfy` no npm usando `NPM_TOKEN`.
...
- `cli/src/version.ts` diverge de `VERSION`.
- `ebooks/VERSION` diverge de `VERSION`.
...
`cli/src/version.ts` repete o valor para o build do executável. Uma alteração de
código ou documentação que gere uma publicação deve atualizar framework, CLI e
ebook juntos. Nunca publique uma edição do ebook com SemVer independente.
